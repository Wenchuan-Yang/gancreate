...
#             'character': 'https://huggingface.co/spaces/mfrashad/CharacterGAN/resolve/main/requirements.txt'
        }

        url = checkpoints[self.outclass]
        download_ckpt(url, outfile) # 下载模型

    def load_model(self):
        checkpoint_root = os.environ.get('GANCONTROL_CHECKPOINT_DIR', Path(__file__).parent / 'checkpoints')
#         checkpoint = Path(checkpoint_root) / f'stylegan2/stylegan2_character_512.pt'
        checkpoint = Path(checkpoint_root) / f'stylegan2/017003.pt'
...
#             self.download_checkpoint(checkpoint)
        ...
        self.model.load_state_dict(ckpt['g'], strict=False)
#         self.latent_avg = ckpt['latent_avg'].to(self.device)
    ...
    def sample_latent(self, n_samples=1, seed=None, truncation=None):
        ...
        rng = np.random.RandomState(seed) # 该方法为np中的伪随机数生成方法，其中的num表示种子，只要种子一致 产生的伪随机数序列即为一致。
        z = torch.from_numpy(
